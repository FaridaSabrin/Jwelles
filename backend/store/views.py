...
from .serializers import (
    ...
    AdminUpdateSupportTicketSerializer,
    CartItemSerializer,
    CategorySerializer,
    ...
    WishlistCollectionDetailSerializer,
    WishlistCollectionPublicSerializer,
    WishlistCollectionItemSerializer,
)
...
def send_otp_email(email, otp):
    """
    Send the OTP to a user's email via the Brevo transactional REST API
    (POST https://api.brevo.com/v3/smtp/email) — a plain HTTPS call, not
    Django's SMTP email backend. Used by both RegisterView and
    ResendOTPView so there is exactly one place that talks to Brevo.

    Returns True only if Brevo responded confirming the email was
    accepted; False for every failure case (bad config, network error,
    Brevo rejection) so callers never report a false "OTP sent".
    """
    if not is_valid_email(email):
        logger.warning("Invalid email format, not sending OTP: %s", email)
        return False

    if not settings.EMAIL_CONFIGURED:
        logger.error(
            "Cannot send OTP email to %s: Brevo is not configured "
            "(BREVO_API_KEY / BREVO_SENDER_EMAIL missing).",
            email,
        )
        return False

    subject = "Verify Your Email - Jwelles"
...
If you did not create an account with Jwelles, please ignore this email.
...
"""
    ...
    logger.info("Attempting to send OTP email to %s via Brevo API", email)
    ...
    try:
        ...
        logger.error(
            "Failed to send OTP email to %s: network error contacting Brevo API: %s",
            email,
            e,
        )
        return False
    ...
    logger.error(
        "Failed to send OTP email to %s: Brevo API responded %s - %s",
        email,
...
class ProductListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        if p.get("occasion"):
            q = q.filter(
                tags__kind="occasion",
                tags__slug__iexact=p["occasion"],
            ).distinct()

        if p.get("style"):
            q = q.filter(
                tags__kind="style",
                tags__slug__iexact=p["style"],
            ).distinct()

        if p.get("tag"):
            q = q.filter(
                tags__slug__iexact=p["tag"]
            ).distinct()
...
class ProductTagListView(generics.ListAPIView):
    ...
    kind (occasion / style / gift / special).
    """
            ...
            return Response(
                {
                    "detail": (
                        ...
                        "email. Please check your email address or try "
                        "resending OTP."
                    )
                },
...
class ResendOTPView(APIView):
    ...
    def post(self, request):
        ...
        logger.info("OTP re-generated for %s", user.email)
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            verified = otp_record.is_verified
    ...
    def post(self, request, user_id):
        try:
            ...
            return Response(
                {
                    "message": (
                        f"Token destroyed for user {user.email}. "
    ...
    def perform_update(self, serializer):
        ...
        if quantity > product.stock:
            raise serializers.ValidationError(
                {
                    "quantity": (
                        "Only available stock may be added."
                    )
                }
            )
...
class AddressListView(APIView):
    ...
    def get(self, request):
        try:
            ...
            print(
                f"Error in AddressListView GET: {str(e)}"
            )
    ...
    def post(self, request):
        try:
            ...
            print(
                f"Error in AddressListView POST: {str(e)}"
            )
...
class CustomizationRequestView(APIView):
    ...
    def get(self, request):
        try:
            requests_list = CustomizationRequest.objects.filter(
                user=request.user
            )
            ...
            print(
                f"Error in CustomizationRequestView GET: {str(e)}"
            )
    ...
    def post(self, request):
        try:
            serializer = CustomizationRequestSerializer(
                data=request.data
            )
            ...
            print(
                f"Error in CustomizationRequestView POST: {str(e)}"
            )
...
class CustomizationRequestDetailView(
    ...
    def get_queryset(self):
        return CustomizationRequest.objects.filter(
            user=self.request.user
        )
...
def calculate_location_discount(address, subtotal):
    matches = [
        d
        for d in LocationDiscount.objects.filter(active=True)
        if d.applies_to(address)
    ]
...
def get_pincode_details(pincode):
    try:
        ...
        if (
            ...
            if (
                ...
                and first_result.get("PostOffice")
            ):
                post_office = first_result["PostOffice"][0]

                return {
                    "pincode": pincode,
                    "city": post_office.get(
                        "District",
                        "",
                    ),
        ...
        print(
            f"Error fetching pincode data from API: {e}"
        )

    except Exception as e:
        print(
            f"Unexpected error in get_pincode_details: {e}"
        )
    ...
    if pincode in fallback_data:
        print(
            f"Using fallback data for pincode: {pincode}"
        )
...
def get_cached_or_fetch_pincode(pincode):
    ...
    try:
        pincode_location = PincodeLocation.objects.get(
            pincode=pincode
        )

        if (
            pincode_location.updated_at
    ...
    if details:
        pincode_location, created = (
            PincodeLocation.objects.update_or_create(
                pincode=pincode,
                defaults={
                    "city": details["city"],
...
def geocode_pincode(pincode_location):
    ...
    city = (
        pincode_location.city
        or pincode_location.district
        or pincode_location.state
    )
...
def get_serviceability(pincode):
    try:
        ...
        pincode_location = get_cached_or_fetch_pincode(
            pincode
        )
        ...
        if not pincode_location:
            return {
                ...
                "serviceable": False,
                "message": (
                    "Invalid pincode or delivery area "
                    "not found."
                ),
            }

        lat, lon = geocode_pincode(
            pincode_location
        )
        ...
        active_areas = DeliveryServiceArea.objects.filter(
            active=True
        )
        ...
        for area in active_areas:
            ...
            if (
                ...
                and distance <= float(area.radius_km)
            ):
                if (
        ...
        pincode_location.is_serviceable = (
        ...
        if matched_area:
            ...
            delivery_start = (
                today
                + timedelta(
                    days=matched_area.delivery_days_min
                )
            )
            ...
            delivery_end = (
                today
                + timedelta(
                    days=matched_area.delivery_days_max
                )
            )
            ...
            return {
                ...
                "delivery_days_min": (
                    matched_area.delivery_days_min
                ),
                "delivery_days_max": (
                    matched_area.delivery_days_max
                ),
                "delivery_start": (
        ...
        print(
            f"Error in get_serviceability: {e}"
        )
...
def validate_coupon(
    ...
    if subtotal < coupon.minimum_order_amount:
        return {
            ...
            "discount_amount": 0,
            "message": (
                f"Minimum order amount of "
                f"₹{coupon.minimum_order_amount} required."
            ),
        }
    ...
    if not serviceability.get("serviceable", False):
        return {
            ...
            "discount_amount": 0,
            "message": (
                "This coupon is not valid for your "
                "delivery location."
            ),
        }

    if (
        coupon.applicable_country
        and serviceability.get("country")
    ):
        if (
            coupon.applicable_country.lower()
            != serviceability["country"].lower()
        ):
            return {
                "valid": False,
                "code": coupon.code,
    ...
    if (
        coupon.applicable_state
        and serviceability.get("state")
    ):
        if (
            coupon.applicable_state.lower()
            != serviceability["state"].lower()
        ):
            return {
                "valid": False,
                "code": coupon.code,
    ...
    if (
        coupon.applicable_city
        and serviceability.get("city")
    ):
        if (
            coupon.applicable_city.lower()
            != serviceability["city"].lower()
        ):
            return {
                "valid": False,
                "code": coupon.code,
    ...
    if (
        coupon.applicable_pincode
        and serviceability.get("pincode")
    ):
        if (
            coupon.applicable_pincode
...
def get_available_coupons(
    ...
    try:
        ...
        print(
            f"Error in get_available_coupons: {e}"
        )
...
class PincodeServiceabilityView(APIView):
    ...
    def get(self, request):
        pincode = request.query_params.get(
            "pincode",
            "",
        )
...
class OrderListCreateView(APIView):
    ...
    def post(self, request):
        ...
        location_discount = calculate_location_discount(
            address,
            subtotal,
        )
        ...
        if coupon_code:
            ...
            if not coupon_result.get(
                "valid",
                False,
            ):
                return Response(
                    {
                        "detail": coupon_result.get(
                            "message",
                            "Invalid coupon.",
                        )
                    },
        ...
        order = Order.objects.create(
            ...
            delivery_days_min=serviceability.get(
                "delivery_days_min",
                3,
            ),
            delivery_days_max=serviceability.get(
                "delivery_days_max",
                6,
            ),
            estimated_delivery_start=serviceability.get(
                "delivery_start",
                today + timedelta(days=3),
            ),
            estimated_delivery_end=serviceability.get(
                "delivery_end",
        ...
        for item in items:
            ...
            OrderItem.objects.create(
                ...
                discount_at_purchase=Decimal("0"),
                subtotal=(
                    product.price
                    * item.quantity
                ),
    ...
    def post(self, request):
        ...
        if not OrderItem.objects.filter(
            ...
            return Response(
                {
                    "detail": (
                        "Only customers with delivered "
                        "purchases may review."
                    )
                },
                status=403,
            )
...
class WishlistCollectionDetailView(APIView):
    ...
    def get(self, request, pk):
        collection, error = self.get_collection(
            pk,
            request.user,
        )
        ...
        serializer = WishlistCollectionDetailSerializer(
            collection
        )
...
class WishlistCollectionItemAddView(APIView):
    ...
    def post(self, request, pk):
        ...
        item = WishlistCollectionItem.objects.create(
            collection=collection,
            product=product,
        )

        serializer = WishlistCollectionItemSerializer(
            item
        )
...
class WishlistCollectionItemRemoveView(APIView):
    ...
    def delete(
        ...
        try:
            item = WishlistCollectionItem.objects.get(
                collection=collection,
                product_id=product_id,
            )
...
class SharedCollectionView(APIView):
    ...
    def get(self, request, token):
        ...
        serializer = WishlistCollectionPublicSerializer(
            collection
        )
...
class SupportTicketDetailView(APIView):
    """
    Retrieve a single ticket — owner only.
    Looked up by the human-readable ticket_id.
    """
...
class SupportMessageCreateView(APIView):
    """
    Send a follow-up message on one of the customer's own tickets.
    """
...
class AdminSupportTicketDetailView(APIView):
    ...
    def patch(self, request, ticket_id):
        ...
        serializer = AdminUpdateSupportTicketSerializer(
            ticket,
            data=request.data,
            partial=True,
        )
...
class AdminSupportMessageCreateView(APIView):
    """
    Admin reply to any customer's ticket.
    """
...
class SupportTicketStatsView(APIView):
    """
    Aggregate counts per status and category
    for the support dashboard.
    """
        ...
        return Response(
            {
                ...
                "awaiting_customer": status_counts.get(
                    "awaiting_customer",
                    0,
                ),
                ...
                "order_issue": category_counts.get(
                    "order_issue",
                    0,
                ),

                "payment_issue": category_counts.get(
                    "payment_issue",
                    0,
                ),

                "product_issue": category_counts.get(
                    "product_issue",
                    0,
                ),

                "delivery_issue": category_counts.get(
                    "delivery_issue",
                    0,
                ),

                "account_issue": category_counts.get(
                    "account_issue",
                    0,
                ),

                "custom_jewellery_issue": category_counts.get(
                    "custom_jewellery_issue",
                    0,
                ),
...
# IMPORTANT:
# This class is intentionally at the top level.
# It must NOT be indented inside SupportTicketStatsView.

class SupportTicketChartDataView(APIView):
    """
    Aggregated data for the Support Dashboard charts — computed in SQL so
    the frontend never has to page through the whole ticket table.
    Every value is derived from existing SupportTicket columns; no new
    fields, no denormalization.
    """
    ...
    def get(self, request):
        ...
        for created, resolved in resolved_pairs:
            if (
                created
                and resolved
...
def send_password_reset_email(email, otp):
    ...
    if not is_valid_email(email):
        logger.warning(
            "Invalid email format, not sending reset OTP: %s",
            email,
        )
        return False
    ...
    if not settings.EMAIL_CONFIGURED:
        logger.error(
            "Cannot send reset OTP to %s: Brevo is not configured.",
            email,
        )
        return False
...
"""
    ...
    try:
        ...
        logger.error(
            "Failed to send reset OTP to %s: network error: %s",
            email,
            e,
        )
        return False
...
class VerifyResetOTPView(APIView):
    ...
    def post(self, request):
        ...
        is_valid, message = otp_record.check_otp(
            otp
        )
...
class ResendResetOTPView(APIView):
    ...
    def post(self, request):
        ...
        logger.info(
            "Password reset OTP re-generated for %s",
            user.email,
        )

...
def send_otp_email(email, otp):
    ...
    if not settings.EMAIL_CONFIGURED:
        # settings.py already logs a CRITICAL/WARNING line at startup when
        # Brevo isn't configured; this bails out before attempting a
        # doomed request and makes the reason explicit for this request.
        logger.error(
...
"""
    ...
    try:
        ...
        # Network error / timeout / DNS failure reaching Brevo itself.
        logger.error(
    ...
    # Brevo rejected the request (bad/expired API key, unverified sender,
    # invalid recipient, rate limit, Brevo-side 5xx, etc). The response
    # body is Brevo's own JSON error ({"code": ..., "message": ...}) and
    # never contains the API key or the OTP, so it's safe to log.
    logger.error(
...
class RegisterView(APIView):
    ...
    def post(self, request):
        ...
        # Create or update OTP record. expires_at has no DB-level default,
        # so get_or_create() needs an explicit value for the INSERT path —
        # set_otp() below immediately overwrites it (and everything else)
        # with the real 10-minute expiry regardless of which branch ran.
        otp_record, created = EmailVerificationOTP.objects.get_or_create(
            user=user,
            defaults={"expires_at": timezone.now() + timedelta(minutes=EmailVerificationOTP.OTP_VALIDITY_MINUTES)},
        )
        otp_record.set_otp(otp)
        logger.info("OTP generated for %s", user.email)

        # Send OTP email - sirf valid email pe jayega
        email_sent = send_otp_email(user.email, otp)

        if not email_sent:
            return Response(
                {
                    "detail": "Account created but failed to send verification email. Please check your email address or try resending OTP."
                },
...
class VerifyEmailOTPView(APIView):
    ...
    def post(self, request):
        ...
        # Activate the account now that the email is confirmed — registration
        # creates the user with is_active=False specifically so login is
        # blocked until this point.
...
class LoginView(APIView):
    ...
    def post(self, request):
        ...
        try:
            user = get_user_model().objects.get(username__iexact=email)
        except get_user_model().DoesNotExist:
            return Response({"detail": "Invalid email or password."}, status=status.HTTP_401_UNAUTHORIZED)

        # Checked directly (instead of via authenticate()) because Django's
        # ModelBackend also refuses inactive users at the password-check
        # step — that would make an unverified account indistinguishable
        # from a wrong password, which is exactly the case we need to
        # report with its own message below.
...
class AdminDestroyTokenView(APIView):
    ...
    def post(self, request, user_id):
        try:
            ...
            return Response({
                "message": f"Token destroyed for user {user.email}. User will be logged out."
            }, status=status.HTTP_200_OK)
        except get_user_model().DoesNotExist:
            return Response(

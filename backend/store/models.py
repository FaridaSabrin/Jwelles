...
class Product(models.Model):
    ...
    def discount_percentage(self):
        if self.original_price and self.original_price > self.price:
            return round(
                (self.original_price - self.price) * 100 / self.original_price,
                2,
            )
        return 0
...
<<<<<<< HEAD
    ...
    def public_queryset(user):
        ...
        badge, product-card hearts, PDP wishlist state, etc.

        A product saved only inside a PRIVATE collection must never be
        inferable from that UI, so it is excluded here at the source
        rather than trusted to frontend filtering. A product that is also
        in a public/shared collection (or in no collection at all) is
        unaffected and keeps the existing public wishlist behaviour.
        """
        private_only_ids = WishlistItem._private_only_product_ids(user)
        queryset = WishlistItem.objects.filter(user=user)
        if private_only_ids:
            queryset = queryset.exclude(product_id__in=private_only_ids)
        return queryset

    @staticmethod
    def _private_only_product_ids(user):
        collection_items = WishlistCollectionItem.objects.filter(collection__user=user)
        private_ids = set(
            collection_items.filter(collection__visibility="private").values_list("product_id", flat=True)
        )
        if not private_ids:
            return set()
        public_ids = set(
            collection_items.exclude(collection__visibility="private").values_list("product_id", flat=True)
        )
...
class LocationDiscount(models.Model):
    ...
    def applies_to(self, address):
        ...
        return (
            ...
            and all(
                not value
                or value.lower() == getattr(address, field).lower()
                for field, value in (
...
class Coupon(models.Model):
    ...
    applicable_country = models.CharField(
        max_length=100,
        blank=True,
    )
    applicable_state = models.CharField(
        max_length=100,
        blank=True,
    )
    applicable_city = models.CharField(
        max_length=100,
        blank=True,
    )
    applicable_pincode = models.CharField(
        max_length=10,
        blank=True,
    )
    minimum_delivery_radius_km = models.DecimalField(
        max_digits=7,
        decimal_places=2,
        null=True,
        blank=True,
...
class OrderItem(models.Model):
    ...
    discount_at_purchase = models.DecimalField(
        max_digits=12,
        decimal_places=2,
        default=0,
    )
...
class EmailVerificationOTP(models.Model):
    ...
    def can_resend(self):
        cooldown_seconds = timedelta(seconds=self.RESEND_COOLDOWN_SECONDS)
        return timezone.now() >= self.last_sent_at + cooldown_seconds

    def set_otp(self, raw_otp):
        self.otp_hash = make_password(raw_otp)
        self.expires_at = timezone.now() + timedelta(minutes=self.OTP_VALIDITY_MINUTES)
        self.attempts = 0
